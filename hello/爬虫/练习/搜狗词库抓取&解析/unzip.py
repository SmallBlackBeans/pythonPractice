...
class ExtSougouScel():
    ...
    def start(self):
        ...
        # 读文件存入queue的线程
        threads.append(
            Thread(target=self.ext_to_queue))

        # 存数据库的线程
        for i in range(10):
            threads.append(
                Thread(target=self.save_to_db))
