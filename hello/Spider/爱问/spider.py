...
class Spider:
    ...
    # 通过网页的页码数来构建网页的URL 这个目前已经失效
    def getPageURLByNum(self, page_num):
        page_url = "https://iask.sina.com.cn/c/187-all-180-new.html"
        return page_url
    ...
    def getQuestions(self, page_num):
        ...
        for question in questions:
            ...
            if info:
                ...
                # 获得插入的问题的自增ID
                insert_id = self.mysql.insertData(mysql.QUESTIONS_TABLE_NAME, ques_dict)
                # 得到最佳答案
                good_ans = ans[0]
                ...
                if good_ans:
                    ...
                    # 插入最佳答案
                    if self.mysql.insertData(mysql.ANSWER_TABLE_NAME, good_ans_dict):
    ...
    def main(self):
        ...
        with open('page.txt', 'w+', encoding='utf-8') as page:  # r+ 如果文件不存在不会创建
            content = page.readline()
            if content:
                start_page = int(content.strip()) - 1
            else:
                start_page = "10"
        print(self.getCurrentTime(), "开始页码", start_page.encode('utf-8'))
